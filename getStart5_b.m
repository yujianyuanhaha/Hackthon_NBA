function Start10 = getStart10(filename)
...
% Game_Lineup = readtable('Game_Lineup_P5.csv');
...
numPeriod = height(Game_Lineup)/10;


Start5 = array2table(cell(numPeriod*2,8));
for i = 1:numPeriod*2
    temp = Game_Lineup( (i-1)*5+[1:5] ,4);
    Game_Lineup( (i-1)*5+1, 4:8) =  array2table(table2array(temp)');
    Start5(i,4:8) =  array2table(table2array(temp)');
    Start5(i,1)= Game_Lineup( (i-1)*5+1,1);
    Start5(i,3)= Game_Lineup( (i-1)*5+1,3);
end
Start5(:,2)=[];
